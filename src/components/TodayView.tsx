...
import { formatWeekdayAndDay, greetingForHour } from "../domain/greeting";
...
export function TodayView({ data, onNavigate, onToggleTask }: { data: AppData; onNavigate: (route: "tasks" | "finance" | "agenda") => void; onToggleTask: (id: string) => void }) {
  ...
  useEffect(() => {
    let active = true;
    fetch("/api/auth/session").then((response) => response.json()).then((session) => { if (active && session?.user?.name) setUserName(session.user.name.split(" ")[0]); }).catch(() => undefined);
    return () => { active = false; };
  }, []);

  const now = new Date();
  const today = todayIso();
  const dailyBudget = computeDailyBudget({ today, transactions: data.transactions, bills: data.bills, categories: data.budgetCategories });
  ...
  const upcomingAppointments = data.appointments.filter((item) => item.date >= today).slice().sort((a, b) => a.date.localeCompare(b.date) || a.time.localeCompare(b.time));
  ...
  return <>
    <PageIntro eyebrow={formatWeekdayAndDay(now)} title={`${greetingForHour(now.getHours())}${userName ? `, ${userName}` : ""}.`} description="Veja o que merece sua atenção e mantenha o restante sob controle." action={<Button onClick={() => onNavigate("tasks")}><Icon name="plus" /> Nova tarefa</Button>} />
