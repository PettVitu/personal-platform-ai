# personal platform AI — documentação operacional

Documentação ativa do aplicativo pessoal privado. A primeira entrega usa frontend, `localStorage` e uma API local demonstrativa em Next.js. Não há autenticação ou banco persistente.

## Documentos

- [Escopo e decisões](./01-escopo-e-decisoes.md)
- [Fluxos e UX](./02-fluxos-e-ux.md)
- [Requisitos funcionais](./03-requisitos-funcionais.md)
- [Backlog do MVP](./04-backlog-mvp.md)
- [Amarildo e IA](./05-ia-e-prompts.md)
- [Segurança e privacidade](./06-seguranca-e-lgpd.md)
- [Roadmap e métricas](./07-roadmap-e-metricas.md)
- [API, repositórios e modo offline](./08-api-e-offline.md)
- [Investimentos e harness](./09-investimentos-e-harness.md)

Materiais históricos do produto anterior foram arquivados/removidos do escopo ativo.
